 ...
 * @author Roman Bauer.
 * @since  1.0.0
 * @since  1.1.0 Added image limit check.
 */
jQuery('document').ready( function( jQuery ){
	...
	if( metaBox.length ) {
		...
		// Get image limit.
		var imgLimitContainer = metaBox.find( '#smfi-img-limit' );
		var imgLimit = parseInt( imgLimitContainer.attr( 'value' ), 10 );

		// Get post id.
		var postID = parseInt( metaBox.find( '#smfi-post-id' ).attr( 'value' ), 10 );
		 ...
		 */
		de.smfi.MediaFrameManager.openMediaFrameOnAddBtnClick( postID, addNewImgBtn, imgContainer, imgLimit, errorContainer );
...
var de = de || {
	smfi : {
		...
		ErrorHandler : {
			...
			/**
			 * Returns the error message for an exceeded image limit.
			 *
			 * It uses the received translated error message from the server. If no error message was transmitted then a default one is used.
			 *
			 * @since      1.1.0
			 ...
			 * @return {string} Error message.
			 */
			getImageLimitExceededErrorMessage : function() {
				if( typeof smfi_translation_object.image_limit_exceeded_error_message !== 'undefined' ) {
					return smfi_translation_object.image_limit_exceeded_error_message;
				} else {
					return getDefaultErrorMessageTxt();
				}
			},
		...
		MediaFrameManager : {
			 ...
			 *
			 * @since      1.0.0
			 * @since      1.1.0 Transmit also data for the image limit check on the server.
			 * @access     private
			 *
	     ...
	     * @param {number} postID  The ID of the current showed post.
	     * @param {jQuery} errorContainer  The error container which can be used for reporting ocurred errors to the user.
			 */
			addNewImg : function( newImgID, imgContainer, postID, errorContainer) {
				...
				var requestData = {
					...
					/*
					* Transmit the received post id of the current post back to server.
					* It will be used on server side to check if it is possible to add more images to the current post.
					*/
					'smfi_post_id': postID,
					...
					'smfi_number_of_images': imgContainer.children( '.smfi-img-wrapper' ).length
				};
			 ...
			 * @param {number} imgLimit  The image limit.
			 * @param {jQuery} errorContainer  The error container which should contain possible the error messages.
			 */
			openMediaFrameOnAddBtnClick : function( postID, btn, imgContainer, imgLimit, errorContainer ) {
				...
				btn.click( function( event ) {
					event.preventDefault();
					/*
					* Check if the given image limit is valid.
					* It is valid if it is an integer and greater than -1. -1 means that unlimited number of pictures can be added.
					*/
					if( Number.isInteger( imgLimit ) && imgLimit >= -1 ) {
						let numberOfImages = imgContainer.children( '.smfi-img-wrapper' ).length;
						/*
						* Allow adding another image if the image limit is set to -1 or the current number of images is below the image limit.
						* A limit of -1 means that unlimited number of pictures can be added.
						*/
						if( imgLimit === -1 || numberOfImages >= 0 && numberOfImages < imgLimit ) {
							mediaFrame.open();
						} else {
							// Image limit is reached -> Show corresponding error message to the user.
							de.smfi.ErrorHandler.showError( errorContainer, de.smfi.ErrorHandler.getImageLimitExceededErrorMessage() );
						}
					} else {
						// Adding image failed because the image limit is invalid for some reason -> Show the default add image error.
						de.smfi.ErrorHandler.showError( errorContainer, de.smfi.ErrorHandler.getAddImageErrorMessage() );
					}
				} );
				...
				mediaFrame.on( 'select', function() {
					...
					if( typeof selectedAttachment.id !== 'undefined' ) {
						...
						mediaFrameManager.addNewImg( selectedAttachment.id, imgContainer, postID, errorContainer );
					} else {
						// Show add image error.
						de.smfi.ErrorHandler.showError( errorContainer, de.smfi.ErrorHandler.getAddImageErrorMessage() );
					}
				} );
