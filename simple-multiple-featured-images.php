...
if ( ! class_exists( 'Simple_Multiple_Featured_Images' ) ) {
	...
	class Simple_Multiple_Featured_Images {
		...
		public function enqueue_admin_scripts() {
			...
			if( $this -> is_supported_post() ) {
				...
				$translation_array = array(
					...
					'image_limit_exceeded_error_message' => $this -> get_image_limit_exceeded_error_message_on_adding_image(),
				);
		 ...
		 *
		 * @since 1.0.0
		 * @since 1.1.0 Added image limit check and updated exception messages.
		 *
		 * @return json JSON with 'newImgHtml' property which contains the html or 'smfiErrorMessage' property which contains the error message
		 * if something go wrong.
		 */
		public function get_img_wrapper_html_by_ajax_as_json() {
			...
			try {
				...
				if( ! check_ajax_referer(	$this -> nonce_manager::NONCE_ACTION_IMG_ADD, $this -> nonce_manager::NONCE_NAME_IMG_ADD, $die_if_invalid ) ) {
					// Get an invalid nonce.
					throw new Exception( $this -> get_invalid_nonce_error_message_on_adding_image() );
				}

				// Check if the received post id belongs to a existing post.
				$post_id = isset( $_POST['smfi_post_id'] ) && is_numeric( $_POST['smfi_post_id'] ) ? intval( $_POST['smfi_post_id'], 10 ) : -1;
				if( get_post_status( $post_id ) === false ) {
					// Received invalid post id for some reason.
					throw new Exception( $this -> get_invalid_post_id_error_message_on_adding_image() );
				}

				// Check if it is possible to add more images.
				$number_of_current_images = isset( $_POST['smfi_number_of_images'] ) && is_numeric( $_POST['smfi_number_of_images'] ) ? absint( $_POST['smfi_number_of_images'], 10 ) : -1;
				$number_of_maximal_images = $this -> get_image_limit(); // returns -1 if there is not image limit.
				$image_limit_exist = $number_of_maximal_images !== -1;
				if( $image_limit_exist && $number_of_current_images >= $number_of_maximal_images ) {
					// It is not allowed to add more images because the limit was reached.
					throw new Exception( $this -> get_image_limit_exceeded_error_message_on_adding_image() . $number_of_current_images );
				}

				// Get requested image by received image id and response the img html to the client.
				$img_id = isset( $_POST['smfi_img_id'] ) && is_numeric( $_POST['smfi_img_id'] ) ? intval( $_POST['smfi_img_id'], 10 ) : -1;
				if( $this -> validator -> is_valid_img_id( intval( $img_id, 10 ) ) ) {
					...
					// Get an invalid image id.
					throw new Exception( $this -> get_invalid_image_id_error_message_on_adding_image() );
				}

			} catch( Exception $exception ) {
					// Response error message to user.
					$this -> send_ajax_error_as_json( $exception -> getMessage() );
			}
		}
		 ...
		 *
		 * @since 1.0.0
		 * @since 1.1.0 Add hidden input field with the post id. It will be used during ajax calls to check if more images can be added to this post.
		 *
		...
		public function echo_meta_box_html( $post ) {
			...
			/*
			* Transmit the ID of the current post to the client.
			* The ID is used on ajax calls to tell the server which specific post the call relates to.
			* This is necessary because by default it is not known at ajax calls which post is currently being processed.
			* The ID ist used, for example, to check if more images can be added to the corresponding post.
			*/
			echo '<input id= "smfi-post-id" name="post-id" type="hidden" value="' . esc_attr( get_the_ID() ) . '">';
		...
		*/
		...
		/**
		 * Returns the meta key which is used inside the database for storing the image data.
		 *
		 * @since 1.0.0
		 *
		...
		*/
		...
		/**
		 * Returns the error message for an exceeded image limit during adding a new image.
		 *
		 * @since 1.1.0
		 *
		 * @return string The escaped and translated error message.
		*/
		private function get_image_limit_exceeded_error_message_on_adding_image() {

			$errorDesc = esc_html__(
							'Adding a picture failed because the limit of the maximum possible pictures is reached. To add more pictures, the limit must be raised. ' ,
							'simple-multiple-featured-images' );
		...
		/**
		 * Returns the error message for an invalid image id during adding a new image.
		 *
		 * @since 1.1.0
		 *
		 * @return string The escaped and translated error message.
		*/
		private function get_invalid_image_id_error_message_on_adding_image() {

			$errorDesc = esc_html__(
							'Adding picture failed because of an invalid picture id. ' ,
							'simple-multiple-featured-images' );
		...
		/**
		 * Returns the error message for an invalid post id during adding a new image.
		 *
		 * @since 1.1.0
		 *
		 * @return string The escaped and translated error message.
		*/
		private function get_invalid_post_id_error_message_on_adding_image() {

			$errorDesc = esc_html__(
							'Adding picture failed because of an invalid post id. ' ,
							'simple-multiple-featured-images' );
		...
		/**
		 * Returns the error message for an invalid nonce during adding a new image.
		 *
		 * @since 1.1.0
		 *
		 * @return string The escaped and translated error message.
		*/
		private function get_invalid_nonce_error_message_on_adding_image() {

			$errorDesc = esc_html__(
							'Adding picture failed because of a security issue. ' ,
							'simple-multiple-featured-images' );
		...
		*/
		private function get_add_new_images_btn_html() {
			$output = '<div class="smfi-btn-container">';
				...
				$output .= '<input id= "smfi-img-limit" name="img-limit" type="hidden" value="' . esc_attr( $this -> get_image_limit() ) . '">';
			$output .= '</div>';
			return $output;
		}
		...
		*/
		...
		/**
		 * Returns the image limit. The addition of new featured images will be prevented when the limit is exceeded.
		 *
		 * @since 1.1.0
		 *
		 * @return int The image limit.
		*/
		private function get_image_limit() {

			/*
			* By default there is not image limit.
			* It is possible to add as many pictures as you like.
			* This will is represented by -1.
			*/
			$default_img_limit = -1;

			/**
			 * Filters the image limit.
			 *
			 * @since 1.1.0
			 *
			 * @param array The number of featured images which should be added.
			 */
			$img_limit = apply_filters( 'smfi_img_limit', $default_img_limit );
			return is_int ( $img_limit ) && $img_limit > 0 ? $img_limit : $default_img_limit;
		}
	}
